...
import { mockDataPerformTracking } from '../../../constants/mockDataPerformTracking'
...
const PerformanceTable = ({ mode, setShowModalDelete, setGoToCreate, setShowModalDetail }: Props) => {
  ...
  const mapDataTable = async () => {
    const res = mockDataPerformTracking.map((item, key) => {
      return {
        ...item,
        rank: key + 1,
  ...
  return (
    <>
                ...
                <tr {...row.getRowProps()} className={`${key % 2 ? "bg-gray-supperLight" : ""} hover:bg-pink transition-all`}>
                  {row.cells.map((cell, key) => {
                    ...
                    return (
                      ...
                      ${props.column.id == "perform" ? "text-left " : ""}
                      `}
                      >
                        {cell.render('Cell')}
                      </td>
      ...
      <FooterTable total={3} size={5} page={0} setCurrentPage={() => ""} />
    </>
  )
}
